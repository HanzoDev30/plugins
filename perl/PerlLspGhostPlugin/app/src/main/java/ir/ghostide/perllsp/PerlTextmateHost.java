...
public final class PerlTextmateHost {

  private static volatile PerlTextmateHost instance;
  ...
  static final String PERL_SCOPE = "source.perl";
  static final String ASSET = "grammars/perl.tmLanguage.json";
  ...
  private PerlTextmateHost(Context androidContext, PluginLogger logger) {
    this.androidContext = androidContext;
    this.logger = logger;
  }

  public static void create(Context androidContext, PluginLogger logger) {
    instance = new PerlTextmateHost(androidContext, logger);
  }

  public static PerlTextmateHost instance() {
    return instance;
  }

  public void loadAsync() {
    Thread worker = new Thread(this::load, "perl-tm-loader");
    worker.start();
  }
    ...
    try {
      GrammarDefinition definition =
          DefaultGrammarDefinition.withGrammarSource(
              new PluginAssetGrammarSource(androidContext, ASSET), "perl-textmate", PERL_SCOPE);
      GrammarRegistry.getInstance().loadGrammars(Arrays.asList(definition));
      ready = true;
      logger.info("Perl TextMate grammar registered (source.perl)");
    } catch (Exception e) {
      logger.error("Failed to register Perl TextMate grammar", e);
    }
  ...
  Language newLanguage(String scopeName) {
    try {
      ...
      logger.warn("perl: cannot create TextMate language for " + scopeName, e);
      return null;
    }
  }
