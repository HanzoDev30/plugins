...
/**
 * Installs and runs the official lua-language-server inside the proot rootfs.
 * Routes .lua files to it via the LSP protocol over stdio.
 */
public final class LuaLspPlugin implements GhostPlugin {

  private static final String INSTALL_SCRIPT_ASSET = "install-lua-lsp.sh";
  private static final String INSTALL_DIR = "/opt/lua-lsp";
  ...
  public List<PluginSetupAction> getSetupActions() {
    ...
    return List.of(
        new PluginSetupAction(
            ...
            command,
            "Downloads the official lua-language-server release (arm64/x64 auto-detected) "
                + "into the proot rootfs and links it into /usr/local/bin. "
                + "Provides completion, diagnostics, hover, goto definition, references and formatting."));
  }
  ...
  public void activate(PluginContext context) {
    ...
    LuaLspProvider provider = new LuaLspProvider(launcher);
    ...
    context
        ...
        .info(
            provider.isInstalled()
                ? "lua-language-server found in rootfs"
                : "lua-language-server not installed yet; run the setup action from the Plugin Manager");
  }
