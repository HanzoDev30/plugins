...
/**
 * Perl code runner implementing {@link CodeRunnerHost}.
 *
 * <p>For {@code .pl/.pm/.t/.psgi} files runs {@code perl <file>} in the host terminal (proot
 * rootfs). For every other file type delegates to the host's default runner.
 *
 * @author Ghost
 */
public final class PerlRunner implements CodeRunnerHost {

  private static final Set<String> EXTENSIONS = Set.of("pl", "pm", "t", "psgi");

  private final PluginContext context;

  public PerlRunner(PluginContext context) {
  ...
  public void runFile(String filePath, boolean asBottomSheet) {
    if (isSupported(filePath)) {
      host().runShell("perl " + shellEscape(filePath), asBottomSheet);
    } else {
