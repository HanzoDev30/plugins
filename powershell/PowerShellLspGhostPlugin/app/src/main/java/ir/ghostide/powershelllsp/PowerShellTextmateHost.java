...
public final class PowerShellTextmateHost {

  private static volatile PowerShellTextmateHost instance;
  ...
  static final String POWERSHELL_SCOPE = "source.powershell";
  static final String ASSET = "grammars/powershell.tmLanguage.json";
  ...
  private PowerShellTextmateHost(Context androidContext, PluginLogger logger) {
    this.androidContext = androidContext;
    this.logger = logger;
  }

  public static void create(Context androidContext, PluginLogger logger) {
    instance = new PowerShellTextmateHost(androidContext, logger);
  }

  public static PowerShellTextmateHost instance() {
    return instance;
  }

  public void loadAsync() {
    Thread worker = new Thread(this::load, "powershell-tm-loader");
    worker.start();
  }
  ...
  private void load() {
    try {
      ...
      ready = true;
      logger.info("PowerShell TextMate grammar registered (source.powershell)");
    } catch (Exception e) {
      logger.error("Failed to register PowerShell TextMate grammar", e);
    }
  ...
  Language newLanguage(String scopeName) {
    try {
      ...
      logger.warn("powershell: cannot create TextMate language for " + scopeName, e);
      return null;
    }
  }
