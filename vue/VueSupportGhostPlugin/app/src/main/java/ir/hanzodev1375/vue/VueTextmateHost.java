...
public final class VueTextmateHost {

  private static volatile VueTextmateHost instance;
  ...
  static final String VUE_SCOPE = "source.vue";
  static final String ASSET = "grammars/vue.tmLanguage.json";
  ...
  private VueTextmateHost(Context androidContext, PluginLogger logger) {
    this.androidContext = androidContext;
    this.logger = logger;
  }

  public static void create(Context androidContext, PluginLogger logger) {
    instance = new VueTextmateHost(androidContext, logger);
  }

  public static VueTextmateHost instance() {
    return instance;
  }

  public void loadAsync() {
    Thread worker = new Thread(this::load, "vue-tm-loader");
    worker.start();
  }
    ...
    try {
      GrammarDefinition definition =
          DefaultGrammarDefinition.withGrammarSource(
              new PluginAssetGrammarSource(androidContext, ASSET), "vue-textmate", VUE_SCOPE);
      GrammarRegistry.getInstance().loadGrammars(Arrays.asList(definition));
      ready = true;
      logger.info("Vue TextMate grammar registered (source.vue)");
    } catch (Exception e) {
      logger.error("Failed to register Vue TextMate grammar", e);
    }
  ...
  private boolean alreadyHandled(CodeEditor editor) {
    ...
    // اگر زبان TextMate یا Wrapper زبانِ LSP (سمت سرور) فعال باشد همان رنگ می دهد؛
    // فقط اگر LSP وصل نشده/زبان خالی باشد زبان ما اعمال می شود.
  ...
  Language newLanguage(String scopeName) {
    try {
      ...
      logger.warn("vue: cannot create TextMate language for " + scopeName, e);
      return null;
    }
  }
