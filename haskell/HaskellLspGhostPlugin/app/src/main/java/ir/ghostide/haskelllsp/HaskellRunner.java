...
/**
 * Haskell code runner implementing {@link CodeRunnerHost}.
 *
 * <p>For {@code .hs/.lhs/.hsc} files runs {@code runghc <file>} in the host terminal (proot
 * rootfs). For every other file type delegates to the host's default runner.
 *
 * @author Ghost
 */
public final class HaskellRunner implements CodeRunnerHost {

  private static final Set<String> EXTENSIONS = Set.of("hs", "lhs", "hsc");

  private final PluginContext context;

  public HaskellRunner(PluginContext context) {
  ...
  public void runFile(String filePath, boolean asBottomSheet) {
    if (isSupported(filePath)) {
      host().runShell("runghc " + shellEscape(filePath), asBottomSheet);
    } else {
