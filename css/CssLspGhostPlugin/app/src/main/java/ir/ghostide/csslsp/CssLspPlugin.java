...
/**
 * Installs and runs vscode-css-language-server inside the proot rootfs.
 * Routes .css files to it via the LSP protocol over stdio.
 */
public final class CssLspPlugin implements GhostPlugin {

  private static final String INSTALL_SCRIPT_ASSET = "install-css-lsp.sh";
  private static final String INSTALL_DIR = "/opt/css-lsp";
  ...
  public List<PluginSetupAction> getSetupActions() {
    ...
    return List.of(
        new PluginSetupAction(
            ...
            "Install CSS language server (vscode-css-language-server)",
            command,
            "Installs Node.js and vscode-langservers-extracted inside the proot rootfs. "
                + "Runs the server through vscode-css-language-server --stdio for .css files. "
                + "Provides completion, diagnostics, hover, formatting and symbol support."));
  }
  ...
  public void activate(PluginContext context) {
    ...
    CssLspProvider provider = new CssLspProvider(launcher);
    ...
    context
        ...
        .info(
            provider.isInstalled()
                ? "vscode-css-language-server wrapper found in rootfs"
                : "vscode-css-language-server not installed yet; run the setup action from the Plugin Manager");
  }
