...
public final class RTextmateHost {

  private static volatile RTextmateHost instance;
  ...
  static final String R_SCOPE = "source.r";
  static final String ASSET = "grammars/r.tmLanguage.json";
  ...
  private RTextmateHost(Context androidContext, PluginLogger logger) {
    this.androidContext = androidContext;
    this.logger = logger;
  }

  public static void create(Context androidContext, PluginLogger logger) {
    instance = new RTextmateHost(androidContext, logger);
  }

  public static RTextmateHost instance() {
    return instance;
  }

  public void loadAsync() {
    Thread worker = new Thread(this::load, "r-tm-loader");
    worker.start();
  }
    ...
    try {
      GrammarDefinition definition =
          DefaultGrammarDefinition.withGrammarSource(
              new PluginAssetGrammarSource(androidContext, ASSET), "r-textmate", R_SCOPE);
      GrammarRegistry.getInstance().loadGrammars(Arrays.asList(definition));
      ready = true;
      logger.info("R TextMate grammar registered (source.r)");
    } catch (Exception e) {
      logger.error("Failed to register R TextMate grammar", e);
    }
  ...
  Language newLanguage(String scopeName) {
    try {
      ...
      logger.warn("r: cannot create TextMate language for " + scopeName, e);
      return null;
    }
  }
