...
/** Routes Lua (.lua) files to lua-language-server inside the proot rootfs. */
public final class LuaLspProvider implements LspServerProvider {

  private static final List<String> CANDIDATE_PATHS =
      Arrays.asList(
          "/usr/local/bin/lua-language-server",
          "/opt/lua-lsp/bin/lua-language-server",
          "/usr/bin/lua-language-server");

  private static final List<String> SERVER_ARGS = List.of();

  private static final Set<String> SUPPORTED_EXTENSIONS = Set.of("lua");

  private final ProotProcessLauncher launcher;

  public LuaLspProvider(ProotProcessLauncher launcher) {
    this.launcher = launcher;
  }

  @Override
  public String getId() {
    return "ir.ghostide.lualls";
  }

  @Override
  public String getDisplayName() {
    return "Lua Language Server (lua-language-server)";
  }
